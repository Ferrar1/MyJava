...
3. 第三范式（3NF）：符合2NF，并且，消除传递依赖（在"A → B → C"的决定关系，则C传递函数依赖于A）,即任何非主属性不依赖于其它非主属性
4. BC范式（BCNF）：符合3NF，并且，任何非主属性不能对主键子集依赖。一般关系型数据库设计中，达到BCNF就可以了！若一个关系达到了第三范式，并且它只有一个候选码，或者它的每个候选码都是单属性，则该关系自然达到BC范式。
