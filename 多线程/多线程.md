...
>讲volatile之前，先讲线程之间如何通信:通过共享变量的方式，AB线程，A更新变量值到主内存，B从主内存读取值。但是现代的处理器为了避免由于IO写入造成停顿而产生的延迟，通过将需要写入内存的数据写缓冲区来临时保存，最后批处理的方式刷新缓冲区。现在就可以讲volatile了，主要靠内存屏障，其他线程想读取该线程的共享变量之前，必须要求该线程写入到内存，即将缓冲区写入内存。这样就可以获得最新值。
