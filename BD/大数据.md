...
2. 复制三台作为集群。每台设备固定IP地址，在主节点上加入这些IP地址映射/etc/hosts。
   - [固定IP地址](https://zhuanlan.zhihu.com/p/40405167)：
     ...
     - 然后进虚拟机，配置固定IP。
   - [ssh免密访问](https://blog.csdn.net/jek123456/article/details/73613828)：A访问B，那么A将自己生成的.pub放到B的authorized_keys里面。然后删掉自己生产的.pub。不过我之前在Gitlab中有本地生产=成过ssh，现在在虚拟机中可以直接用。
