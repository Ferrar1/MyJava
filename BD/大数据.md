...
3. 本人在Ubuntu下安装。
## Ubuntu下安装
1. 利用ubuntu自带的virtualBox，安装一个Ubuntu后设置启动模式为3，即命令行启动。
2. 复制三台作为集群。每台设备固定IP地址，在主节点上加入这些IP地址映射/etc/hosts。固定IP地址[操作](https://blog.csdn.net/wang5990302/article/details/80282322),中途可能遇到的问题是选择仅主机网络时，界面名称没法选择：点击右上角的全局工具，新建就行了。
