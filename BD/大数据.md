...
2. 复制三台作为集群。每台设备固定IP地址，在主节点上加入这些IP地址映射/etc/hosts。
   - [固定IP地址](http://www.jiaoben.net/wenzhang/106437.html)：
     - 首先要在VirtualBox界面-设置-网络中，选择Host-Only然后设置网卡。中途可能遇到的问题是选择仅主机网络时，界面名称也就是网卡没法选择：点击右上角的全局工具，新建就行了。要三个虚拟机，于是新建三个网卡。
     - 然后进虚拟机，配置固定IP
