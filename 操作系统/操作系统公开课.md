...
### 内核级线程
   ...
   - 在内核中切换switch_to，以一套栈为单位进行切换。switch_to过程：**通过TCB(该线程当前运行信息)找到内核栈指针，然后通过ret切换到某个内核程序，最后再用CS:PC切到用户程序。**切换的目的是：主要还是执行用户程序，进入内核只是完成切换。
       ...
       <img src="https://github.com/xuzhuang1996/MyJava/blob/master/img/操作系统/21.PNG" width=65% height=65% />
   - 一些概念
      - cs指向段基址,
      - 段寄存器SS存放栈顶段地址
      - 段寄存器SP存放栈顶的偏移地址
      ...
      - IRet:中断返回
   - 整个流程：
      - <img src="https://github.com/xuzhuang1996/MyJava/blob/master/img/操作系统/22.PNG" width=65% height=65% />
