...
2. 复制三台作为集群。每台设备固定IP地址，在主节点上加入这些IP地址映射/etc/hosts。
   - [固定IP地址](https://zhuanlan.zhihu.com/p/40405167)：
     ...
     - 然后进虚拟机，配置固定IP。
   - ssh免密访问：A访问B，那么A将自己生成的.pub放到B的authorized_keys里面。这里直接在root用户下创建authorized_keys文件，然后写进去。因为后面用的是root用户启动，因此不要用自己的用户，使用前root密码记得改，百度，改好后就不管了，否则就算密码对，也登不进去。不过我之前在Gitlab中有本地生产=成过ssh，现在在虚拟机中可以直接用。另外
