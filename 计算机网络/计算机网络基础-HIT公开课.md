...
> 5. 为何4次挥手：tcp是全双工模式，接收到FIN时意味将没有数据再发来，但是还是可以继续发送数据。1当客户端发送FIN，2服务器收到后返回ACK：告诉Client端，你的请求我收到了，但是我还没准备好，请继续你等我的消息；当Server端确定数据已发送完成，3则向Client端发送FIN报文，"告诉Client端，好了，我这边数据发完了，准备好关闭连接了"因此ACK与FIN不能一起发送。故需要四步握手。
> 6. 讲TCP的这个，首先介绍确认机制ACK，以及随机数SYN的原因。然后开始介绍传输过程。
